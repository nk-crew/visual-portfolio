...
$doc.on('extendClass.vpf', (event, VP) => {
	...
	VP.prototype.destroySwiper = function () {
		const $parent = this.$items_wrap.parent();
		const $thumbsParent = this.$slider_thumbnails_wrap.length
			? this.$slider_thumbnails_wrap.parent()
			: false;
		...
		if (ThumbsSwiper) {
			...
			this.$slider_thumbnails_wrap.removeClass('swiper-wrapper');
			this.$slider_thumbnails_wrap.children().removeClass('swiper-slide');
		...
		if (SliderSwiper) {
			...
			this.$items_wrap.removeClass('swiper-wrapper');
			this.$items_wrap.children().removeClass('swiper-slide');
