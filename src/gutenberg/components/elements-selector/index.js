...
function ElementsSelectorOptions(props) {
  const { location, locationData, value, onChange, options, optionName, parentProps } = props;
  ...
  return (
    <>
      ...
      {isOpen ? (
        <Modal
          title={`${options[optionName] ? options[optionName].title : optionName} ${__(
            'Settings',
            '@@text_domain'
          )}`}
          onRequestClose={(e) => {
...
export default class ElementsSelector extends Component {
  ...
  getLocationData(location) {
    ...
    const availableAlign =
      locations[location] && locations[location].align ? locations[location].align : [];
