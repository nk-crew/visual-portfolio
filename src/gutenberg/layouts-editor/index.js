...
class UpdateEditor extends Component {
   ...
   */
  checkIfPostEdited() {
    const { isSavingPost, isAutosavingPost, blockData, editPost } = this.props;
  ...
  saveMetaOnPostUpdate() {
    const { isSavingPost, isAutosavingPost, postId, blockData } = this.props;
...
registerPlugin('vpf-saved-layouts-editor', {
  render: compose(
    withSelect((select) => {
      const { isSavingPost, isAutosavingPost, getCurrentPostId, getEditorSettings } =
        select('core/editor');
    ...
    withDispatch((dispatch) => {
      const { selectBlock, insertBlocks, resetBlocks } = dispatch('core/block-editor');
