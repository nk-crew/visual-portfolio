...
class Visual_Portfolio_Assets {
    ...
    public function register_scripts() {
        ...
        if ( apply_filters( 'vpf_enqueue_plugin_isotope', true ) ) {
            self::register_script( 'isotope', 'src/assets/vendor/isotope-layout/dist/isotope.pkgd.min', array( 'jquery' ), '3.0.6' );
        }

        // fjGallery.
        if ( apply_filters( 'vpf_enqueue_plugin_flickr_justified_gallery', true ) ) {
            self::register_script( 'flickr-justified-gallery', 'src/assets/vendor/flickr-justified-gallery/dist/fjGallery.min', array( 'jquery' ), '2.1.2' );
        }

        // PhotoSwipe.
        if ( 'photoswipe' === $popup_vendor && apply_filters( 'vpf_enqueue_plugin_photoswipe', true ) ) {
            self::register_style( 'photoswipe', 'src/assets/vendor/photoswipe/dist/photoswipe', array(), '4.1.3' );
            self::register_style( 'photoswipe-default-skin', 'src/assets/vendor/photoswipe/dist/default-skin/default-skin', array( 'photoswipe' ), '4.1.3' );
            self::register_script( 'photoswipe', 'src/assets/vendor/photoswipe/dist/photoswipe.min', array( 'jquery' ), '4.1.3' );
            self::register_script( 'photoswipe-ui-default', 'src/assets/vendor/photoswipe/dist/photoswipe-ui-default.min', array( 'jquery', 'photoswipe' ), '4.1.3' );

            // Fancybox.
        } elseif ( 'fancybox' === $popup_vendor && apply_filters( 'vpf_enqueue_plugin_fancybox', true ) ) {
            self::register_style( 'fancybox', 'src/assets/vendor/fancybox/dist/jquery.fancybox.min', array(), '3.5.7' );
            self::register_script( 'fancybox', 'src/assets/vendor/fancybox/dist/jquery.fancybox.min', array( 'jquery' ), '3.5.7' );
        }

        // Swiper.
        if ( apply_filters( 'vpf_enqueue_plugin_swiper', true ) ) {
            self::register_style( 'swiper', 'src/assets/vendor/swiper/swiper-bundle.min', array(), '8.4.7' );
            self::register_script( 'swiper', 'src/assets/vendor/swiper/swiper-bundle.min', array(), '8.4.7' );
        }

        // Simplebar.
        if ( apply_filters( 'vpf_enqueue_plugin_simplebar', true ) ) {
            self::register_style( 'simplebar', 'src/assets/vendor/simplebar/dist/simplebar.min', array(), '5.3.0' );
            self::register_script( 'simplebar', 'src/assets/vendor/simplebar/dist/simplebar.min', array(), '5.3.0' );
        }
        ...
        if ( apply_filters( 'vpf_enqueue_plugin_lazysizes', true ) ) {
            ...
            self::register_script( 'lazysizes', 'src/assets/vendor/lazysizes/lazysizes.min', array( 'lazysizes-config', 'lazysizes-object-fit-cover', 'lazysizes-swiper-duplicates-load' ), '5.3.2' );
        }
